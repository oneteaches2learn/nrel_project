...
//			  << "\n   Y[HF]:    \t" << Y[HF_ID]
	...
	if (lam_unit == "poise") {lam_unit2cgs = 1;}
	else if (lam_unit == "Pa_s") {lam_unit2cgs = 10;}
	else if (lam_unit == "mPa_s") {lam_unit2cgs = 0.01;}
	else if (lam_unit == "uPa_s") {lam_unit2cgs = 1e-5;}
	else {lam_unit2cgs = 10;}
    ...
    result_dlamdrho.open         ("PelePhys_DV_dlamdrho.txt");
    ...
    result_dlamdrho         << substance << " dlam_drho [" << lam_unit << "/" << R_unit << "] PelePhys\n";
    ...
    for (int i = 0; i < P_steps; i++) {
        ...
        for (int j = 0; j < T_steps; j++) {
            ...
            trans.transport(
                ...
                R, Y, Ddiag, chi_mix, mu, xi, lam, dmudrho, dlamdrho, ltransparm);
			...
			result_dlamdrho			<< dlamdrho / lam_unit2cgs * R_unit2cgs;
            if (j != T_steps-1) {
